...
public class TeamAssessmentFragment extends Fragment implements AsyncResponse {

    private List<AssessmentModel> employeeList = new ArrayList<>();
    private RecyclerView rvEmployeeInfo;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.frag_team_assessment, container, false);

        rvEmployeeInfo = (RecyclerView) rootView.findViewById(R.id.rvEmployeeInfo);

        RecyclerView.LayoutManager mLayoutManager = new LinearLayoutManager(getActivity());
        rvEmployeeInfo.setLayoutManager(mLayoutManager);
        rvEmployeeInfo.setItemAnimator(new DefaultItemAnimator());
        getTeamAssessment();
        return rootView;
    }

    public void getTeamAssessment() {
        if (BasicActivity.isOnline(getActivity())) {
            String url = CWUrls.GET_EBAT_TEAM_ASSESSMENT;
    ...
    public void processFinish(String output, int status_code, String url, int type) {
        if (url.contains(CWUrls.GET_EBAT_TEAM_ASSESSMENT)) {
                ...
                if (main.has("data")) {
                    ...
                    if (system_array != null) {
                        for (int i = 0; i < system_array.length(); i++) {
                            ...
                            assessment.getAppraisee().setName(appraiseeObj.optString("displayName"));
            ...
            if (status.equals("success")) {
                mAdapter = new AssessmentAdapter(getActivity(), employeeList, false);

                rvEmployeeInfo.setAdapter(mAdapter);
            } else {
